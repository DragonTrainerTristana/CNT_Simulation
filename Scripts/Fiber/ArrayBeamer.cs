...
public class ArrayBeamer : MonoBehaviour
{
    public GameObject prefabCnt;

    public int numCnt = 100;

    private int cntParentNum;
    int cntNameNum;
    ...
    private void FixedUpdate()
    {
        ...
        if (stateTime == false && startTime >= 3)
        {
    ...
    void SetParentCnt()
    { // 부모 오브젝트 할당 (정육면체 한 면)
        parentCnt[0] = GameObject.Find("AllFiber");
        parentCnt[1] = GameObject.Find("AllFiber");
        parentCnt[2] = GameObject.Find("AllFiber");
        parentCnt[3] = GameObject.Find("AllFiber");
        parentCnt[4] = GameObject.Find("AllFiber");
        parentCnt[5] = GameObject.Find("AllFiber");
    }
    void SetArrayRange()
    {
        //CNT_ONE Range
    ...
    void GenerateCNT()
    {
        for (int i = 0; i < 6; i++)
        {
            for (int j = 0; j < numCnt; j++)
            {
                ...
                cnt.tag = "CNT";
                cntArray[cntNameNum-1].GetComponent<Beamer>().myParentNum = cntParentNum;
                cntParentNum++;
